...
;(function($) {
	...
	var methods = {
		init: function(options) {
			return this.each(function() {
		        ...
		        $steps.each(function(index) {
		        	...
		        	if (index == 0) {
						...
						if(index == 1){
							methods.createContinueButton.call($this, index);
						}
						if(index > 1){
        	...
        	$('<a/>', { id: id + '-back-' + index, href: 'javascript:void(0);', 'class': 'button-back btn btn-warning', html: opt.backLabel }).click(function() {
        		if (!opt.back || methods.execute.call($this, opt.back, index - 1)) {
        			methods.step.call($this, (index - 1) + 1);
        		}
            }).appendTo($('#' + id + '-buttons-' + index));
        },createContinueButton: function(index) {
			var $this	= this,
				id		= this.attr('id'),
				opt		= this.data('options');
			$('<a/>', { id: 'continues', href: 'javascript:void(0);', 'class': 'btn btn-warning button-back','onclick':'continueConnect()',html: opt.continueLabel }).click(function() {
				if (!opt.continue || methods.execute.call($this, opt.continue, index - 1)) {
					methods.step.call($this, (index - 1) + 1);
				}
			}).appendTo($('#' + id + '-buttons-' + index));
        	...
        	$('<a/>', { id: id + '-next-' + index, href: 'javascript:void(0);', 'class': 'button-next  btn btn-warning', html: opt.nextLabel }).click(function() {
